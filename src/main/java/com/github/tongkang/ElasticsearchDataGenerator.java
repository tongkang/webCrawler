...
public class ElasticsearchDataGenerator {
        ...
        try (RestHighLevelClient client = new RestHighLevelClient(RestClient.builder(new HttpHost("localhost", 9200, "http")))) {
            for (int i = 0; i < 4000; i++) {
                ...
                for (News news : newsFromMySQL) {
                    IndexRequest request = new IndexRequest("new");
                ...
                System.out.println("Current thread; " + Thread.currentThread().getName() + "finishes" + i + ":" + bulkResponse.status());
            }
    ...
    private static List<News> getNewsFromMySQL(SqlSessionFactory sqlSessionFactory) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            return session.selectList("com.github.tongkang.MockMapper.selectNews");
        }
    }
}
